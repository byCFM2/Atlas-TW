--[[
	...
	Documentation: http://www.wowace.com/index.php/AceDBa-2.0
	SVN: http://svn.wowace.com/root/trunk/Ace2/AceDBa-2.0
	Description: Mixin to allow for fast, clean, and featureful saved variable
	access.
	Dependencies: AceLibrary, AceOO-2.0, AceEvent-2.0
]]

local MAJOR_VERSION = "AceDBa-2.0"
local MINOR_VERSION = "$Revision: 17799 $"
...
AceDBa.CHAR_ID = charID
AceDBa.REALM_ID = realmID
AceDBa.CLASS_ID = classID

AceDBa.FACTION = faction
AceDBa.REALM = realm
AceDBa.NAME = UnitName("player")
...
function AceDBa:InitializeDB(addonName)
	local db = self.db

	if not db then
		if addonName then
			AceDBa.addonsLoaded[addonName] = true
		end
		return
	end
	...
	if self['AceDBa-profile-copylist'] then
		local t = self['AceDBa-profile-copylist']
		for k,v in pairs(t) do
			t[k] = nil
		end
	...
	if self['AceDBa-profile-list'] then
		local t = self['AceDBa-profile-list']
		for k,v in pairs(t) do
			t[k] = nil
		end
...
function AceDBa:OnEmbedInitialize(target, name)
	if name then
...
function AceDBa:RegisterDB(name, charName)
	AceDBa:argCheck(name, 2, "string")
	AceDBa:argCheck(charName, 3, "string", "nil")
	if self.db then
		AceDBa:error("Cannot call \"RegisterDB\" if self.db is set.")
	end
	...
	if AceDBa.addonsLoaded[addonName] then
		AceDBa.InitializeDB(self, addonName)
	else
		AceDBa.addonsToBeInitialized[self] = addonName
	end
	AceDBa.registry[self] = true
end

function AceDBa:RegisterDefaults(kind, defaults, a3)
	local name
	if a3 then
		name, kind, defaults = kind, defaults, a3
		AceDBa:argCheck(name, 2, "string")
		AceDBa:argCheck(kind, 3, "string")
		AceDBa:argCheck(defaults, 4, "table")
		if kind ~= "char" and kind ~= "class" and kind ~= "profile" and kind ~= "account" and kind ~= "realm" then
			AceDBa:error("Bad argument #3 to `RegisterDefaults' (\"char\", \"class\", \"profile\", \"account\", or \"realm\" expected, got %q)", kind)
		end
	else
		AceDBa:argCheck(kind, 2, "string")
		AceDBa:argCheck(defaults, 3, "table")
		if kind ~= "char" and kind ~= "class" and kind ~= "profile" and kind ~= "account" and kind ~= "realm" then
			AceDBa:error("Bad argument #2 to `RegisterDefaults' (\"char\", \"class\", \"profile\", \"account\", or \"realm\" expected, got %q)", kind)
		end
	end
	if type(self.db) ~= "table" or type(self.db.name) ~= "string" then
		AceDBa:error("Cannot call \"RegisterDefaults\" unless \"RegisterDB\" has been previously called.")
	end
	local db
	if name then
		local namespace = self:AcquireDBNamespace(name)
		if namespace.defaults and namespace.defaults[kind] then
			AceDBa:error("\"RegisterDefaults\" has already been called for %q::%q.", name, kind)
		end
		db = namespace
	else
		if self.db.defaults and self.db.defaults[kind] then
			AceDBa:error("\"RegisterDefaults\" has already been called for %q.", kind)
		end
		db = self.db
	end
...
function AceDBa:ResetDB(kind)
	AceDBa:argCheck(kind, 2, "nil", "string")
	if not self.db or not self.db.raw then
		AceDBa:error("Cannot call \"ResetDB\" before \"RegisterDB\" has been called and before \"ADDON_LOADED\" has been fired.")
	end
	local db = self.db
	if kind == nil then
		if db.charName then
...
function AceDBa:SetProfile(name, copyFrom)
	AceDBa:argCheck(name, 2, "string")
	AceDBa:argCheck(copyFrom, 3, "string", "nil")
	if not self.db or not self.db.raw then
		AceDBa:error("Cannot call \"SetProfile\" before \"RegisterDB\" has been called and before \"ADDON_LOADED\" has been fired.")
	end
	local db = self.db
	local copy = false
	...
	if copyFrom then
		if string.sub(lowerCopyFrom, 1, 5) == "char/" then
			AceDBa:assert(lowerName == "char", "If argument #3 starts with `char/', argument #2 must be `char'")
		elseif string.sub(lowerCopyFrom, 1, 6) == "realm/" then
			AceDBa:assert(lowerName == "realm", "If argument #3 starts with `realm/', argument #2 must be `realm'")
		elseif string.sub(lowerCopyFrom, 1, 6) == "class/" then
			AceDBa:assert(lowerName == "class", "If argument #3 starts with `class/', argument #2 must be `class'")
		else
			AceDBa:assert(lowerName ~= "char" and lowerName ~= "realm" and lowerName ~= "class", "If argument #3 does not start with a special prefix, that prefix cannot be copied to.")
		end
		if not db.raw.profiles or not db.raw.profiles[copyFrom] then
			AceDBa:error("Cannot copy profile %q, it does not exist.", copyFrom)
		elseif (string.sub(lowerName, 1, 5) == "char/" and string.sub(lowerName, 6) == string.lower(charID)) or (string.sub(lowerName, 1, 6) == "realm/" and string.sub(lowerName, 7) == string.lower(realmID)) or (string.sub(lowerName, 1, 6) == "class/" and string.sub(lowerName, 7) == string.lower(classID)) then
			AceDBa:error("Cannot copy profile %q, it is currently in use.", name)
		end
	end
	...
	if self['AceDBa-profile-list'] then
		if not self['AceDBa-profile-list'][name] then
			self['AceDBa-profile-list'][name] = name
		end
	end
	if Hewdrop then
		Hewdrop:Refresh(1)
...
function AceDBa:ToggleActive(state)
	AceDBa:argCheck(state, 2, "boolean", "nil")
	if not self.db or not self.db.raw then
		AceDBa:error("Cannot call \"ToggleActive\" before \"RegisterDB\" has been called and before \"ADDON_LOADED\" has been fired.")
	end
	local db = self.db
...
function AceDBa:embed(target)
	self.super.embed(self, target)
	if not AceEvent then
		AceDBa:error(MAJOR_VERSION .. " requires AceEvent-2.0")
	end
end

function AceDBa:ADDON_LOADED(name)
	AceDBa.addonsLoaded[name] = true
	for addon, addonName in pairs(AceDBa.addonsToBeInitialized) do
		if name == addonName then
			AceDBa.InitializeDB(addon, name)
			AceDBa.addonsToBeInitialized[addon] = nil
		end
	end
end

function AceDBa:PLAYER_LOGOUT()
	for addon in pairs(AceDBa.registry) do
		local db = addon.db
		if db then
			setmetatable(db, nil)
...
function AceDBa:AcquireDBNamespace(name)
	AceDBa:argCheck(name, 2, "string")
	local db = self.db
	if not db then
		AceDBa:error("Cannot call `AcquireDBNamespace' before `RegisterDB' has been called.", 2)
	end
...
function AceDBa:GetAceOptionsDataTable(target)
	if not target['AceDBa-profile-list'] then
		target['AceDBa-profile-list'] = setmetatable({}, caseInsensitive_mt)
		local t = target['AceDBa-profile-list']
		for k,v in pairs(t) do
			t[k] = nil
		end
		t.char = CHARACTER .. charID
	...
	if not target['AceDBa-profile-copylist'] then
		target['AceDBa-profile-copylist'] = setmetatable({}, caseInsensitive_mt)
		if target.db and target.db.raw then
			local t = target['AceDBa-profile-copylist']
			local db = target.db
		...
		profile = {
			...
			args = {
				choose = {
					...
					get = "GetProfile",
					set = "SetProfile",
					validate = target['AceDBa-profile-list']
				},
				copy = {
					...
					get = "GetProfile",
					set = "SetProfile",
					validate = target['AceDBa-profile-copylist'],
					disabled = function()
						return not next(target['AceDBa-profile-copylist'])
					end,
				},
				other = {
...
AceLibrary:Register(AceDBa, MAJOR_VERSION, MINOR_VERSION, activate, nil, external)
AceDBa = AceLibrary(MAJOR_VERSION)