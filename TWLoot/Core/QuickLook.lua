--- @file QuickLook.lua
--- @brief Manages the Quick Look functionality for AtlasTWLoot.
		...
		local setOptions = function()
			for i = 1, 6 do
				...
				Hewdrop:AddLine(
					...
					"func", function()
						local dataID = AtlasTWLootItemsFrame and AtlasTWLootItemsFrame.StoredElement or nil
						local storedMenu = AtlasTWLootItemsFrame and AtlasTWLootItemsFrame.StoredMenu or nil
						if not AtlasTWCharDB["QuickLooks"] then AtlasTWCharDB["QuickLooks"] = {} end
						local titleText = nil
						if AtlasTWLoot_LootPageName and AtlasTWLoot_LootPageName:GetText() and AtlasTWLoot_LootPageName:GetText() ~= "" then
							titleText = AtlasTWLoot_LootPageName:GetText()
...
function AtlasTW.QuickLook.RefreshButtons()
	...
	while i<7 do
		...
		local btnPanel = _G["AtlasTWLootPanel_Preset"..i]
		local btnItems = _G["AtlasTWLootItemsFrame_Preset"..i]
		local enable = false
		if entry then
