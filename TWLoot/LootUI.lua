...
-- @usage AtlasTWLoot_ApplyParentTemplate(myFrame)
---
local function AtlasTWLoot_ApplyParentTemplate(frame)
    frame:SetWidth(236)
    frame:SetHeight(28)
...
-- @function AtlasTWLoot_ApplyNavigationButtonTemplate
-- @param button Button - The button to apply template to
-- @param buttonType string - Type of navigation button ("prev" or "next")
-- @usage AtlasTWLoot_ApplyNavigationButtonTemplate(myButton, "prev")
---
local function AtlasTWLoot_ApplyNavigationButtonTemplate(button, buttonType)
    button:SetWidth(32)
    button:SetHeight(32)
...
---
-- Function to apply AtlasTWLootContainerItemTemplate properties
-- @function AtlasTWLoot_ApplyContainerItemTemplate
-- @param button Button - The button to apply container item template to
-- @usage AtlasTWLoot_ApplyContainerItemTemplate(myButton)
---
function AtlasTWLoot_ApplyContainerItemTemplate(button)
    button:SetWidth(40)
    button:SetHeight(40)
...
-- @function AtlasTWLoot_CreateSearchElements
-- @param frame Frame - The parent frame to attach search elements to
-- @return table - Table containing all created search elements
-- @usage local searchElements = AtlasTWLoot_CreateSearchElements(parentFrame)
---
local function AtlasTWLoot_CreateSearchElements(frame)
    -- Search Box
    local searchBox = CreateFrame("EditBox", "AtlasTWLootSearchBox", frame, "InputBoxTemplate")
...
---
-- Create tooltips for AtlasTWLoot
-- @function AtlasTWLoot_CreateTooltips
...
-- @function AtlasTWLoot_CreateButtonFromTemplate
...
-- @usage local button = AtlasTWLoot_CreateButtonFromTemplate("MyButton", parentFrame, "AtlasTWLootItem_Template")
---
function AtlasTWLoot_CreateButtonFromTemplate(name, parent, templateType)
    local button

    if templateType == "AtlasTWLootItem_Template" then
...
-- @function AtlasTWLoot_CreateItemsFrame
-- @return Frame - The created items frame with all UI elements
-- @usage local itemsFrame = AtlasTWLoot_CreateItemsFrame()
---
local function AtlasTWLoot_CreateItemsFrame()
    ...
    -- Create scrollbar
    local scrollBar = CreateFrame("ScrollFrame", "AtlasTWLootScrollBar", frame, "FauxScrollFrameTemplate")
    ...
    local pageName = frame:CreateFontString("AtlasTWLoot_LootPageName", "OVERLAY", "GameFontHighlightLarge")
    ...
    local quickLooks = frame:CreateFontString("AtlasTWLoot_QuickLooks", "OVERLAY", "GameFontNormal")
    ...
    -- QuickLooks button
    local quickLooksButton = CreateFrame("Button", "AtlasTWLootQuickLooksButton", frame)
    ...
    menuButton:SetScript("OnClick", function()
        if AtlasTWLoot_Hewdrop:IsOpen() then
            AtlasTWLoot_Hewdrop:Close()
        else
            AtlasTWLoot_Hewdrop:Open(this)
        end
    end)

    -- Font strings
    AtlasTWLoot_CreateFontStrings(frame)

    return frame
end

---
-- Create the main AtlasTWLoot panel with navigation buttons
-- @function AtlasTWLoot_CreatePanel
-- @return Frame - The created panel frame with all navigation elements
-- @usage local panel = AtlasTWLoot_CreatePanel()
---
local function AtlasTWLoot_CreatePanel()
    ...
    -- Preset buttons
    AtlasTWLoot_CreatePresetButtons(frame)
    -- Search elements
    AtlasTWLoot_CreateSearchElements(frame)
