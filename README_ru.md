...
11) Локализация
- Atlas-TW использует модульную систему локализации на основе пространств имён
- Структура:
  - `Locales/LocalizationFramework.lua` — ядро системы локализации
  - `Locales/[locale]/` — языковые файлы (enUS, deDE, esES, ptBR, zhCN)
  - Каждая локаль содержит 9 модулей: Core, Zones, Bosses, Classes, Factions, Spells, ItemSets, MapData, QuestData
- Автоматический откат на английский язык при отсутствии перевода
- Для добавления/обновления переводов: редактируйте соответствующий файл в `Locales/[locale]/`

12) Технические детали
- Не требует внешних библиотек Babble (заменены модульной системой)
- Все данные локализации загружаются через `Locales/locales.xml`
